...
![](/cloud/project/Diapositive1.JPG)
...
- les different fonctions pour construire les bonnes regions. @Kevin : j'ai pas compris ce que tu entendais par là
- la programmation S3 pour traiter différement les données entrées par l'utilisateur selon le type de sortie choisie.
</ul>


(répartition taches (qui a fait quoi), difficultés du au fait que certaines parties dépendaient du travail des autres : pb de synchronisation)
...
<ul>
- le dataframe : la valeur selectionnée dependant du nom de la colonne, identifiée par le nom du pays, et de la ligne, identifiée par le nom de la composante à traduire
...
- @Kevin : as tu fait autrement ? si oui, peux tu l'expliquer ?
</ul>
...
- render... ou reactive : Reactive met à jour une variable en fonction d'input. Cette variable peut ensuite être utilisée dans un render... qui est ensuite utilisée dans un widget
@Kevin : apeux tu vérifier/détailler ?
- IfElse : @Kevin : apeux tu détailler ?
</ul>

## Répétition d'une instruction

Pour répéter une instruction, nous sommes habitué à utiliser une boucle for ou while. Pour Shiny, nous avons trouvé qu'il était plus facile d'utiliser un fonction lapply.
...
- Passer par une programmation RCPP permettrait peut être d'optimiser le code
 ...
 sujet pas le plus pertinant pour R : lenteur application shiny, pas beaucoup de stat à nuancer car
